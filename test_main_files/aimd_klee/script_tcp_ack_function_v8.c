...
extern struct tcp_congestion_ops tcp_reno; //tcp reno member of tcp_congestion_ops
...
{
	struct sk_buff *skb;
	skb = malloc (sizeof (*skb)); //skb pointer
	if (skb == NULL) {
	...
	tcp_set_congestion_control(sk, &tcp_reno); //cca algo
...
//	u32 num_rtt = 1;
	...
	klee_assume (sym_cwnd >= 1 && sym_cwnd <= 1000); //made change here 100 -> 1000
...
//			printf ("Undoing------\n");
			...
			if (three_dup_acks) {
				...
				tp->lost_out-2; //made change
				tp->retrans_out-2;//made change
...
//				printf ("Aggregate last group of update normal packets------\n");
				...
				prior_cwnd = tp->snd_cwnd;
				return_status = aggregate_normal_events (sk, skb);
				TCP_SKB_CB(skb)->seq -= agg_update_packets_acked[num_special_events];//made change
